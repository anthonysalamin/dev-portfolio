!(function () {
    ...
    class o {
      ...
      reloadPagination() {
        this.hasPrevious
          ...
          document.body.dispatchEvent(
            new CustomEvent("pagination-change", { bubbles: !0 })
          );
      }
    }
    function s() {
      return (s = t(function* () {
        var e = new a("search");
        return yield e.getCachedOrLiveURL("/_data/search-index.json");
    ...
    class u {
      constructor(e) {
        var n = this;
        (this.form = e),
          (this.contentType = e.dataset.contentType || "application/json"),
          (this.success = this.form.parentElement.querySelector(".w-form-done")),
          (this.error = this.form.parentElement.querySelector(".w-form-fail")),
      ...
      sendAsUrlEncoded() {
        ...
        return t(function* () {
          yield e.sendForm({
            body: new URLSearchParams(new FormData(e.form)).toString()
          });
        })();
      }
      sendAsJSON() {
        var e = this;
        return t(function* () {
          var t = Object.fromEntries(new FormData(e.form).entries());
          yield e.sendForm({ body: JSON.stringify(t) });
        })();
      }
      sendForm(e) {
        var r = this;
        return t(function* () {
          (r.error.style.display = "none"),
            r.form.querySelectorAll('[type="submit"]').forEach((e) => {
              e.dataset.value || (e.dataset.value = e.value),
                (e.value = e.dataset.wait);
            });
          try {
            (yield fetch(
              r.form.action,
              (function (e) {
                ...
                { method: "POST", headers: { "Content-Type": r.contentType } },
                e
              )
            )).ok
              ? ((r.form.style.display = "none"),
                (r.success.style.display = "inherit"),
                r.form.dataset.redirect &&
                  setTimeout(() => {
                    window.location = r.form.dataset.redirect;
                  }))
    ...
    document
      ...
      document.querySelectorAll(".w-pagination-wrapper").forEach((e) => {
        new o(e);
      }),
      document.querySelectorAll("form").forEach((e) => {
        "true" === e.dataset.ajax && new u(e);
      }),
      "/search/" !== window.location.pathname &&
        (function () {
          return s.apply(this, arguments);
        })().then(() => console.log("yo Search System Enabled")),
      ...
      document.dispatchEvent(new CustomEvent("udesly.loaded")),
      document.documentElement.classList.add("udesly-loaded");
  })();
